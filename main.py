...
color_cld_path=os.path.join(folder_path,"cld_color")
remark_file=os.path.join(remark_path, "remarks.txt")
pinned_file = os.path.join(pinned_path, "ImportantNote.txt")
cld_color=os.path.join(color_cld_path,"cld_color.txt")
wdw_color=os.path.join(color_cld_path,"wdw_color.txt")
...
os.makedirs(color_cld_path,exist_ok=True)
...
def apply_dark_mode():
    ...
    CURRENT_ENTRY_BG = DARK_ENTRY

    # Apply to all major frames and widgets
    for frame in [home, card_frame, calendar_tab, notes_tab, timer_tab, todo_tab, note_btn_frame, button_frame]:
        frame.configure(bg=CURRENT_BG)

    home_title.configure(bg=CURRENT_BG, fg=CURRENT_TEXT)
    greeting_label.configure(bg=CURRENT_BG, fg=CURRENT_TEXT)
    file_listbox.configure(bg=CURRENT_ENTRY_BG, fg=CURRENT_TEXT)
    ...
    # Update ttk style (tab color etc.)
    style = ttk.Style()
    style.configure('TNotebook', background=CURRENT_BG)
    style.configure('TNotebook.Tab',padding=[20, 10],font=("Segoe UI", 10, "bold"),borderwidth=0)
    style.map("TNotebook.Tab", background=[("selected", "#666666")],expand=[("selected", [1, 1, 1, 0])])
...
def apply_light_mode():
    ...
    # Light theme tabs
    style = ttk.Style()
    style.configure('TNotebook', background=WHITE_BG)
    style.configure('TNotebook.Tab',foreground=TEXT_COLOR, padding=[20, 10], font=("Segoe UI", 10, "bold"), borderwidth=0)
    style.map("TNotebook.Tab", background=[("selected", BLUE_SELECTED)], expand=[("selected", [1, 1, 1, 0])])

    saved_theme = load_color()
    if saved_theme:
        cal.config(**saved_theme)

def toggle_theme():# a button show when it was light when dark mode
    global current_mode
...
home = tk.Frame(notebook, bg=CURRENT_BG)
notebook.add(home, text="🏠 Home")

theme_toggle_btn = tk.Button(top_frame, text="🌙 Dark Mode", command=toggle_theme)
theme_toggle_btn.pack(side='right', padx=20, pady=10)
...
greeting_label = tk.Label(home,text=greeting_text,font=("Segoe UI", 14, "bold"),fg=CURRENT_TEXT,bg=CURRENT_BG)
...
#get result from the txt file and load the item
...
def open_trash_bin():
    ...
    def delete_all_files():
        confirm = messagebox.askyesno("Delete All", "Are you sure you want to permanently delete all files in the Trash Bin?")# clear all the file 
        if confirm:
...
#note tab
notes_tab = tk.Frame(notebook, bg=CURRENT_BG)
notebook.add(notes_tab, text="📝 Notes")

# Search bar
search_entry = tk.Entry(notes_tab, font=FONT_TEXT, bg=CURRENT_ENTRY_BG, fg=CURRENT_TEXT, insertbackground=CURRENT_TEXT)
...
# Listbox for files
file_listbox = tk.Listbox(notes_tab, font=FONT_TEXT, bg=CURRENT_ENTRY_BG, fg=CURRENT_TEXT)
file_listbox.place(relx=0.05, rely=0.12, relwidth=0.4, relheight=0.8)

# Button Frame
note_btn_frame = tk.Frame(notes_tab, bg=CURRENT_BG)
...
btn_export = ttk.Button(note_btn_frame, text="Export Notes", command=lambda: export_notes())
...
remark_text = tk.Text(calendar_tab, width=50, height=6, font=("Arial", 12),bg=CURRENT_ENTRY_BG, fg=CURRENT_TEXT, insertbackground=CURRENT_TEXT)
remark_text.pack(pady=10)

# Theme function
def save_color(theme_data):
    with open(cld_color,"w")as f:
